...
from src.db import validate_login, add_user, get_user_name
...
class CreateAccount(tk.Frame):
    def __init__(self, parent, controller):
        ...
        pincodeLabel = tk.Label(self, text="pincode", font=controller.title_font).grid(row=9, column=0)
        self.pincode = tk.IntVar()
        pincodeEntry = tk.Entry(self, textvariable=self.pincode, font=controller.title_font).grid(row=9, column=1)
...
class LogIn(tk.Frame):
    def __init__(self, parent, controller):
        ...
        label = tk.Label(self, text="Welcome to nice_bank!", font=controller.title_font).grid(row=0,column=0)

        # controller.geometry('400x150')  
        self.controller.title("nice_bank")
...
def raise_window(title: str, message: str):
    top= tk.Tk()
    top.title(title)
    label = tk.Label(top, text= message, font=('Arial 18 bold'))
    label.pack()